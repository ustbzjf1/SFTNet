    ...
    def __init__(self, num_in, num_out, g=1, stride=1,norm=None,dilation=None):
        ...
        for i in range(3):
            self.conv3x3x3_m1.append(
                ...
                DilatedConv3DBlock(num_mid, num_out, kernel_size=(3, 3, 3), stride=dilation[i], g=g,
                                   d=(1,1,1), norm=norm)
            )
...
class MFNet_SFT_adaptive(nn.Module): # softmax
    ...
    def forward(self, x):
        ...
        temp2 = self.encoder3(x2)
        temp22 = self.SFT3(temp2)
        x3 = self.encoder_block3(temp22)  # H//8 down
        temp3 = self.encoder4(x3)
        print('temp3:', temp3.shape)
        temp33 = self.SFT4(temp3)
        x4 = self.encoder_block4(temp33)  # H//16
...
class DilatedMFNet_T(MFNet_SFT_adaptive): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.logger_layer7 = None
        self.logger_layer8 = None
        ...
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            # (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            MFUnit_add2(channels*3, channels*3, g=groups, stride=1, norm=norm, dilation=[1, 2, 3], layerlogs=self.logger_layer7),
            MFUnit_add2(channels*3, channels*2, g=groups, stride=1, norm=norm, dilation=[1, 2, 3], layerlogs=self.logger_layer8), # before 2x upsample, we 2x reduce the channels respondingly.
        )
...
if __name__ == '__main__':
    ...
    model = DilatedMFNet_T(c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False)
    model.cuda(device)
    y = model(x)
    print(y.shape)
