...
class DilatedMFNet_F(BaseModel): # softmax
    # [96]   Flops:  17.091G  &  Params: 2.2M
    # [112]  Flops:  21.749G  &  Params: 2.98M
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self, c=4,n=32,channels=96, groups=16,norm='bn', num_classes=4,output_func='sigmoid'):
...
if __name__ == '__main__':
    ...
    x = torch.rand((1, 4, 240, 240, 160),device=device) # [bsize,channels,H,W,Depth] [bsize,channels,H,W,D]
    # model = MF_VNet_16x_Dilated_A(c=4, groups=16, norm='bn', num_classes=4)
    model = DilatedMFNet_SFT(c=4, groups=16, norm='bn', num_classes=4)
    model.cuda(device)
    y = model(x)
    print(y.shape)
