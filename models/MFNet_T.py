...
class MFNet_SFT2_adaptive(nn.Module): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.SFT3 = SFT_adaptive(256, 16, 16, normalize=self.norm_GCN, mode='ft')
        ...
        self.SFT4 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='sft')
...
class MFNet_SFT4_cross_adaptive(nn.Module): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.SFT31 = SFT_adaptive(256, 16, 16, normalize=self.norm_GCN, mode='sft')
        self.encoder32 = nn.Sequential(
        MFUnit_A(channels * 2, channels * 2, g=groups, stride=1, norm=norm),
        MFUnit_A(channels * 2, channels * 2, g=groups, stride=1, norm=norm))
        
        self.SFT32 = SFT_adaptive(256, 16, 16, normalize=self.norm_GCN, mode='sft')
        self.encoder_block3 = nn.Sequential(
            # (MFUnit_A(channels, channels*2, g=groups, stride=2, norm=norm)), # H//8
            MFUnit_A(channels * 2, channels * 2, g=groups, stride=1, norm=norm),# Dilated Conv 3
            MFUnit_A(channels * 2, channels * 2, g=groups, stride=1, norm=norm)
        ...
        self.SFT41 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='sft')
        self.encoder42 = nn.Sequential(
        MFUnit_A(channels * 3, channels * 3, g=groups, stride=1, norm=norm),
        MFUnit_A(channels * 3, channels * 3, g=groups, stride=1, norm=norm))
        self.SFT42 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='sft')
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            # (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            MFUnit_A(channels*3, channels*3, g=groups, stride=1, norm=norm),
            MFUnit_A(channels*3, channels*2, g=groups, stride=1, norm=norm), # before 2x upsample, we 2x reduce the channels respondingly.
...
class MFNet_SFT3_adaptive(nn.Module): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.SFT2 = SFT_adaptive(128, 32, 32, normalize=self.norm_GCN, mode='sft')
        self.encoder_block2 = nn.Sequential(
            #MFUnit_A(n, channels, g=groups, stride=2, norm=norm),  # H//4 down
            MFUnit_A(channels, channels, g=groups, stride=1, norm=norm),  # Dilated Conv 3
            MFUnit_A(channels, channels, g=groups, stride=1, norm=norm)
...
class MFNet_SFT6_adaptive(nn.Module): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.encoder2 = MFUnit_A(n, channels, g=groups, stride=2, norm=norm)
        self.SFT21 = SFT_adaptive(128, 32, 32, normalize=self.norm_GCN, mode='sft')
        self.SFT22 = SFT_adaptive(128, 32, 32, normalize=self.norm_GCN, mode='sft')
        self.encoder_block2 = nn.Sequential(
            #MFUnit_A(n, channels, g=groups, stride=2, norm=norm),  # H//4 down
            MFUnit_A(channels, channels, g=groups, stride=1, norm=norm),  # Dilated Conv 3
            MFUnit_A(channels, channels, g=groups, stride=1, norm=norm)
...
class MFNet_SFT_adaptive_behind(nn.Module): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.SFT4 = SFT_adaptive(256, 8, 8, normalize=self.norm_GCN, mode='sft')
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            (MFUnit_A(channels*3, channels*3, g=groups, stride=1, norm=norm)),
            (MFUnit_A(channels*3, channels*2, g=groups, stride=1, norm=norm)), # before 2x upsample, we 2x reduce the channels respondingly.
...
class DilatedMFNet_sft_stage3(MFNet_SFT2_adaptive): # softmax
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        self.encoder3 = MFUnit_add2(channels, channels*2, g=groups, stride=2, norm=norm,dilation=[1,2,3],layerlogs=self.logger_layer4)
        self.SFT3 = SFT_adaptive(256, 16, 16, normalize=self.norm_GCN, mode='sft')
...
class MFNet_s_ins4_adaptive(nn.Module): # softmax
    # [96]   Flops:  13.361G  &  Params: 1.81M
    # [112]  Flops:  16.759G  &  Params: 2.46M
    # [128]  Flops:  20.611G  &  Params: 3.19M
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        #  feature maps : 8x8x8 without dilated conv.
        self.encoder4 = MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)
        self.SFT4 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='s')
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            # (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            (MFUnit_A(channels*3, channels*3, g=groups, stride=1, norm=norm)),
            (MFUnit_A(channels*3, channels*2, g=groups, stride=1, norm=norm)), # before 2x upsample, we 2x reduce the channels respondingly.
...
class MFNet_f_ins4_adaptive(nn.Module): # softmax
    # [96]   Flops:  13.361G  &  Params: 1.81M
    # [112]  Flops:  16.759G  &  Params: 2.46M
    # [128]  Flops:  20.611G  &  Params: 3.19M
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        #  feature maps : 8x8x8 without dilated conv.
        self.encoder4 = MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)
        self.SFT4 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='f')
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            # (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            (MFUnit_A(channels*3, channels*3, g=groups, stride=1, norm=norm)),
            (MFUnit_A(channels*3, channels*2, g=groups, stride=1, norm=norm)), # before 2x upsample, we 2x reduce the channels respondingly.
...
class MFNet_t_ins4_adaptive(nn.Module): # softmax
    # [96]   Flops:  13.361G  &  Params: 1.81M
    # [112]  Flops:  16.759G  &  Params: 2.46M
    # [128]  Flops:  20.611G  &  Params: 3.19M
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='sync_bn', num_classes=4, output_func='softmax', norm_GCN=False):
        ...
        #  feature maps : 8x8x8 without dilated conv.
        self.encoder4 = MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)
        self.SFT4 = SFT_adaptive(384, 8, 8, normalize=self.norm_GCN, mode='t')
        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            # (MFUnit_A(channels*2, channels*3, g=groups, stride=2, norm=norm)),  # H//16
            (MFUnit_A(channels*3, channels*3, g=groups, stride=1, norm=norm)),
            (MFUnit_A(channels*3, channels*2, g=groups, stride=1, norm=norm)), # before 2x upsample, we 2x reduce the channels respondingly.
...
if __name__ == '__main__':
    ...
    model = MFNet_SFT_adaptive(c=4, num_classes=4)
    model.cuda(device)
    y = model(x)
    print(y.shape)
