...
# train
train_set = {
        'root': os.path.join(config.root, config.train_dir),
        'flist': 'all.txt',
        'has_label': True
        }
####
...
valid_set = {
        'root': os.path.join(config.root, config.valid_dir),
        'flist': 'valid.txt',
        'has_label': False
        }

test_set = {
        'root': '/data2/liuxiaopeng/Data/BraTS2019/Test',
        'flist': 'test.txt',
        'has_label': False
        }
...
def process_f32b0(path, has_label=True):
    ...
    mask = images.sum(-1) > 0
    for k in range(4):
        # (1319790,) (1319833,) (1319833,) (1319811,)
