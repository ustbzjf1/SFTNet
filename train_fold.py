...
def main():
    ...
    x_axis = range(config.start_epoch, config.end_epoch)
    loss_all = []
    accuracy_1 = []
    accuracy_2 = []
    accuracy_4 = []
    ...
    for epoch in range(config.start_epoch, config.end_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            logging.info('Epoch: {}_Iter:{}  loss: {:.5f} || 1:{:.4f} | 2:{:.4f} | 4:{:.4f} ||'.format(epoch, i, loss, loss1, loss2, loss3))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            
            loss_epoch.append(loss.cpu().detach().numpy())
            area1.append(loss1.cpu().detach().numpy())
            area2.append(loss2.cpu().detach().numpy())
            area4.append(loss3.cpu().detach().numpy())

        loss_all.append(np.mean(loss_epoch))
        accuracy_1.append(np.mean(area1))
        accuracy_2.append(np.mean(area2))
        accuracy_4.append(np.mean(area4))
    ...
    plt.plot(x_axis, accuracy_1, color='blue', linewidth=1, label="accuracy_1")
    plt.plot(x_axis, accuracy_2, color='purple', linewidth=1, label="accuracy_2")
    plt.plot(x_axis, accuracy_4, color='green', linewidth=1, label="accuracy_4")
    plt.title('Training process')
    plt.ylabel('loss/accuracy')
    plt.legend()
    save_path = './train_process/' + config.experiment + config.data + '.jpg'
